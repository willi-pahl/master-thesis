...
\begin{lstlisting}[language=html,label=lst:hello_word_js_client,caption={Hello World in clientseitigen JavaScript}]
<!DOCTYPE html>
<html>
<head>
...
\texttt{Klare Struktur und übersichtliches Layout:}
\texttt{
	\begin{itemize}
		\item Logische Reihenfolge: Die Felder sollten in einer logischen Reihenfolge an\-ge\-ord\-net sein (z.B. zuerst Name, dann Straße, dann PLZ und Ort).
		\item Gruppenbildung: Ähnliche Felder können zusammengefasst werden (z.B. Vorname und Nachname in einer Zeile).
		\item Ausreichend Platz: Jedes Feld sollte ausreichend groß sein, um die Eingabe zu erleichtern.
		\item Beschriftungen: Jedes Feld muss eine klare und präzise Beschriftung haben.
		\item Platzhaltertexte: Platzhaltertexte in den Feldern können den Nutzern Hin\-wei\-se geben, was sie eingeben sollen (z.B. "Musterstraße 12").
	\end{itemize}
}
\texttt{Typen von Eingabefeldern:}
\texttt{
	\begin{itemize}
		\item Textfelder: Für Namen, Straßennamen, Orte usw.
		\item Numerische Felder: Für Postleitzahlen, Telefonnummern.
		\item Dropdown-Listen: Für Länder, Bundesländer oder bestimmte Aus\-wahl\-mög\-lich\-kei\-ten.
		\item Checkboxen: Für Mehrfachauswahlen (z.B. Zustelloptionen).
	\end{itemize}
}
\texttt{Pflichtfelder:}
\texttt{
	\begin{itemize}
		\item Kennzeichnung: Machen Sie deutlich, welche Felder zwingend ausgefüllt wer\-den müssen (z.B. durch einen Sternchen).
		\item Begründung: Erklären Sie kurz, warum bestimmte Informationen benötigt wer\-den.
	\end{itemize}
}
\texttt{Validierung:}
\texttt{
	\begin{itemize}
		\item Echtzeit-Validierung: Überprüfen Sie die Eingaben direkt beim Ausfüllen, um Fehler sofort anzuzeigen.
		\item Typische Fehler: Überprüfen auf korrekte Formate (z.B. E-Mail-Adresse, Te\-le\-fon\-num\-mer).
		\item Fehlermeldungen: Geben Sie klare und hilfreiche Fehlermeldungen aus.
	\end{itemize}
}
\texttt{Datenschutzhinweise:}
\texttt{
	\begin{itemize}
		\item Transparenz: Informieren Sie die Nutzer darüber, welche Daten erhoben werden und wie diese genutzt wer\-den.
		\item Einwilligung: Holen Sie eine ausdrückliche Einwilligung der Nutzer ein, be\-vor Sie die Daten verarbeiten.
		\item Datenschutzbestimmungen: Verlinken Sie auf Ihre Datenschutzbestimmungen.
	\end{itemize}
}
\texttt{Zusätzliche Tipps:}
\texttt{
	\begin{itemize}
		\item Benutzerfreundlichkeit: Achten Sie auf eine einfache und intuitive Be\-die\-nung.
		\item Responsive Design: Das Formular sollte auf verschiedenen Geräten (Desktop, Tablet, Smartphone) gut aussehen und funktionieren.
		\item Barrierefreiheit: Achten Sie auf eine barrierefreie Gestaltung (z.B. aus\-rei\-chen\-de Kontraste, Beschriftungen für Formularelemente).
		\item Sicherheit: Schützen Sie die übermittelten Daten durch Verschlüsselung.
	\end{itemize}
}
\texttt{Beispiel eines Formulars:}

\texttt{Wird in einem neuen Fenster geöffnet \href{https://formidableforms.com/research-based-tips-improve-contact-form-conversions/}{formidableforms.com}}

\texttt{Tools zur Erstellung von Formularen:}
\texttt{
	\begin{itemize}
		\item Website-Baukästen: Viele Website-Baukästen (z.B. Wix, Squarespace) bieten integrierte Formular-Builder.
		\item Formular-Plugins: Für WordPress gibt es zahlreiche Plugins (z.B. Gravity Forms, Contact Form 7).
		\item Programmierung: Für individuelle Lösungen können Sie HTML, CSS und Ja\-va\-Script verwenden.
	\end{itemize}
}
\texttt{Wichtiger Hinweis:}

\texttt{Die konkrete Gestaltung Ihres Formulars hängt von Ihren individuellen An\-for\-der\-ung\-en und den geltenden Datenschutzbestimmungen ab. Es ist ratsam, sich recht\-lich beraten zu lassen, um sicherzustellen, dass Ihr Formular alle ge\-setz\-lich\-en Anforderungen erfüllt.}\vspace{0.2cm}
\end{tcolorbox}

Daraus wurden die Anforderungen für das Formular abgeleitet und ein weiterer Prompt an die LLM abgesetzt.\vspace{0.2cm}
...
<body>
  ...
  form.addEventListener('submit', (event) => {
    ...
    // Wenn alle Felder gültig sind, kannst du hier die Daten
    // weiterverarbeiten z.B. an einen Server senden oder lokal speichern
...
\end{lstlisting}
\texttt{Erklärung der wichtigsten Punkte:}
\texttt{
	\begin{itemize}
		\item Struktur und Layout: Das Formular ist klar strukturiert und die Felder lo\-gisch gruppiert.
		\item Beschriftung und Platzhalter: Jedes Feld hat eine klare Beschriftung und teilweise Platzhaltertexte.
		\item Pflichtfelder: Pflichtfelder sind mit einem Sternchen gekennzeichnet und das required-Attribut sorgt dafür, dass diese Felder ausgefüllt werden müs\-sen.
		\item Responsive Design: Das CSS sorgt dafür, dass das Formular sich an ver\-schie\-dene Bildschirmgrößen anpasst.
		\item Echtzeit-Validierung: Das JavaScript-Script prüft bei jedem Versuch, das Formular abzuschicken, ob alle Pflichtfelder ausgefüllt sind. Du kannst die\-se Validierung noch erweitern, um beispielsweise die Eingabeformate zu über\-prü\-fen.
		\item Datenverarbeitung: Die gesammelten Daten werden im JavaScript-Code aus\-ge\-ge\-ben. Hier kannst du sie an einen Server senden oder anderweitig ver\-ar\-bei\-ten.
	\end{itemize}
}
\texttt{Verbesserungen:}
\texttt{
	\begin{itemize}
		\item Zusätzliche Validierungen: Du kannst weitere Validierungen hinzufügen, z.B. um zu prüfen, ob die Postleitzahl numerisch ist oder ob die E-Mail-Adresse ein gültiges Format hat.
		\item Benutzerfreundlichkeit: Du kannst die Fehlermeldungen noch genauer gestalten und dem Benutzer Hinweise geben, wie er die Fehler beheben kann.
		\item Styling: Passe das CSS an, um das Formular optisch ansprechender zu ge\-stal\-ten.
		\item Barrierefreiheit: Achte darauf, dass das Formular auch für Menschen mit Be\-hin\-der\-ung\-en zugänglich ist (z.B. durch Verwendung von ARIA-Attributen).
	\end{itemize}
}
\texttt{Integration in deine Webseite:}\vspace{0.2cm}
\texttt{Kopiere den HTML-Code in den entsprechenden Abschnitt deiner Webseite. Passe den CSS-Stil an dein vorhandenes Design an und füge das JavaScript in den <head>-Be\-reich oder vor dem schließenden </body>-Tag ein.}\vspace{0.2cm}
\texttt{Wichtiger Hinweis:}
\texttt{
	\begin{itemize}
		\item Datenschutz: Achte darauf, dass du die Datenschutzbestimmungen einhältst und die Nutzer darüber informierst, wie ihre Daten verwendet werden.
		\item Sicherheit: Wenn du die Daten an einen Server sendest, solltest du eine si\-chere Verbindung (HTTPS) verwenden und die Daten verschlüsseln.
		\item Mit diesem Formular hast du eine solide Grundlage für ein be\-nut\-zer\-freund\-lich\-es und funktionales Adressformular.
	\end{itemize}
}
\end{tcolorbox}

Das Ergebnis ist ein Formular, das die geforderten Felder umsetzt und dies grundlegend validiert. Die Abbildung \ref{img:address_form_create_by_gemini} zeigt das Ergebnis.

\begin{figure}[!ht]
	\includegraphics[width=0.7\textwidth]{content/chapter_basics/images/result_form_gemini.eps}
	\centering
	\caption{Erzeugtes Formular mit Gemini}
	\label{img:address_form_create_by_gemini}
\end{figure}
